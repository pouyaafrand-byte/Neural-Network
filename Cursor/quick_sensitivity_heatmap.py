"""
Quick Sensitivity Analysis: Create heatmap of validation loss
Testing: Layers (2,3,4) × Neurons (10,15,20) × Activations (ELU, LEAKY_RELU, RELU, TANH)
"""
...
class QuickSensitivityAnalysis:
    ...
    def create_model(self, num_layers, neurons_per_layer, activation='relu'):
    ...
    def train_and_evaluate(self, model, epochs=50, batch_size=32, verbose=0):
        ...
        # Callbacks - shorter patience for speed
    ...
    def run_sensitivity_analysis(self):
        ...
        print("\n" + "="*80)
        print("QUICK SENSITIVITY ANALYSIS: VALIDATION LOSS HEATMAP")
        print("="*80)
        
        # Define parameter ranges (layers 2-10 for comprehensive analysis)
        num_layers_list = [2, 3, 4, 5, 6, 7, 8, 9, 10]
        neurons_list = [10, 15, 20]
        ...
        print("Using reduced epochs for speed...\n")
        ...
        for num_layers in num_layers_list:
            for neurons in neurons_list:
                for act_name, act_func in activation_functions.items():
                    ...
                    print(f"[{count}/{total_combinations}] {arch_name} + {act_name}...", end=' ')
                    
                    try:
                        # Create model
                        model = self.create_model(num_layers, neurons, activation=act_func)
                        
                        # Train and evaluate (fewer epochs for speed)
                        val_loss = self.train_and_evaluate(model, epochs=50, verbose=0)
        ...
        csv_filename = f'quick_sensitivity_results_{timestamp}.csv'
    ...
    def create_heatmap(self):
        ...
        sns.heatmap(
            ...
            linecolor='gray',
            vmin=0,
            vmax=max(10, pivot_data.max().max())
        )
        ...
        # Save figure
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'quick_sensitivity_heatmap_{timestamp}.png'
...
def main():
    ...
    analyzer = QuickSensitivityAnalysis('DATA.xlsx')
